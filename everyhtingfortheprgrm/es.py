...
def shutdown_program(event=None):
    """Close the whole program when Escape is pressed."""
    global program_running, nyan_running, pet_running
...
def on_global_key_press(key):
    """Catch Escape even when another window has keyboard focus."""
    if key == keyboard.Key.esc:
        try:
            root.after(0, shutdown_program)
        except Exception:
            pass
        return False
...
def start_keyboard_listener():
    with keyboard.Listener(on_press=on_global_key_press) as listener:
        listener.join()
...
def on_click(
    ...
    if not pressed or not program_running:
        return
...
# Press Escape anywhere to immediately close the program.
root.bind_all("<Escape>", shutdown_program)
